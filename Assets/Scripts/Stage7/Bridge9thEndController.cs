...
public class Bridge9thEndController : MonoBehaviour {
    ...
    void Update()
    {
        if (RotateController)
        {
            targetRotation = Quaternion.Euler(0, -180, RotateAngle * state + Bridge9thController.RotateAngle * Bridge9thController.state) * Quaternion.identity;
