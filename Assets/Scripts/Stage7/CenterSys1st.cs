...
public class CenterSys1st : MonoBehaviour {

    private Quaternion targetRotation;    //声明旋转目标角度
    public float RotateAngle = 90;       //定义每次旋转的角度
    public static int count;                  //声明一个量记录到目标角度需要进行旋转RotateAngle度的个数
    ...
    void Update()
    {
        if (RotateController)
        {
            targetRotation = Quaternion.Euler(0, RotateAngle * count, 0) * Quaternion.identity;  //给旋转目标值赋值，由于只有Y轴动，所以目标值应是  (旋转角(RotateAngle)*需要旋转的个数(count)+origionY(物体初始Y轴旋转角))*Quarternion.identity(四元数的初始值,记住写就行！)
