...
public class Bridge6thEndController : MonoBehaviour {
    ...
    void Update()
    {
        if (RotateController)
        {
            targetRotation = Quaternion.Euler(0, -90, RotateAngle * state + Bridge6thController.RotateAngle * Bridge6thController.state) * Quaternion.identity;
