...
public class Bridge5thEndController : MonoBehaviour {
    ...
    void Update()
    {
        if (RotateController)
        {
            targetRotation = Quaternion.Euler(0, -90, RotateAngle * state + Bridge5thController.RotateAngle * Bridge5thController.state) * Quaternion.identity;
