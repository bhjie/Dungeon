...
public class Bridge4thEndController : MonoBehaviour {
    ...
    void Update()
    {
        if (RotateController)
        {
            targetRotation = Quaternion.Euler(0, -180, RotateAngle * state + Bridge4thController.RotateAngle * Bridge4thController.state) * Quaternion.identity;
