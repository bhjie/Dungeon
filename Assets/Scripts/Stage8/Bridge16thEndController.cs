...
public class Bridge16thEndController : MonoBehaviour {
    ...
    void Update()
    {
        if (RotateController)
        {
            targetRotation = Quaternion.Euler(0, -180, RotateAngle * state + Bridge16thController.RotateAngle * Bridge16thController.state) * Quaternion.identity;
